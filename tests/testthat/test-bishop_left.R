...
test_that("bishop_left() - Tests for errors", {
  ...
  expect_error(bishop_left(nodes_qu),
               paste0("The bishop left movement is not designed to work ", 
                      "through quadrats only. Please use pawn() instead."),
               fixed = TRUE)
  
  expect_error(bishop_left(nodes_tr),
               paste0("The bishop left movement is not designed to work ", 
...
test_that("bishop_left() - Tests for success (2D network)", {
  
  expect_silent({
    neighbors <- bishop_left(nodes,
                             focus    = "3-3",
                             degree   = 1,
                             directed = TRUE,
                             reverse  = FALSE, 
  ...
  expect_equal(neighbors[ , "node"], c("2-4"))
  ...
  expect_equal(neighbors[ , "node"], c("1-5", "2-4"))
  ...
  expect_equal(ncol(neighbors), 4L)
  expect_equal(nrow(neighbors), 1L)
  expect_equal(neighbors[ , "node"], c("4-2"))
  ...
  expect_equal(ncol(neighbors), 4L)
  expect_equal(nrow(neighbors), 2L)
  expect_equal(neighbors[ , "node"], c("2-4", "4-2"))
  ...
  expect_equal(neighbors[ , "node"], c("2-4", "3-3", "4-2"))
})
