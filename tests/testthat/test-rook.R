...
test_that("rook() - Tests for errors", {
  ...
  expect_error(rook(nodes_qu),
               paste0("The rook movement is not designed to work through ", 
                      "quadrats only. Please use pawn() instead."),
               fixed = TRUE)
  
  expect_error(rook(nodes_tr),
               paste0("The rook movement is not designed to work through ", 
...
test_that("rook() - Tests for success (2D network)", {
  
  expect_silent({
    neighbors <- rook(nodes,
                      focus    = "3-3",
                      degree   = 1,
                      directed = TRUE,
                      reverse  = FALSE, 
  ...
  expect_equal(neighbors[ , "node"], c("2-3", "3-4", "4-3"))
  ...
  expect_equal(class(neighbors), "data.frame")
  expect_equal(ncol(neighbors), 4L)
  expect_equal(nrow(neighbors), 6L)
  expect_equal(neighbors[ , "node"], c("1-3", "2-3", "3-4", "3-5","4-3", "5-3"))
  ...
  expect_equal(ncol(neighbors), 4L)
  expect_equal(nrow(neighbors), 3L)
  expect_equal(neighbors[ , "node"], c("2-3", "3-2", "4-3"))
  ...
  expect_equal(neighbors[ , "node"], c("2-3", "3-2", "3-4", "4-3"))
  ...
  expect_equal(neighbors[ , "node"], c("2-3", "3-2", "3-3", "3-4", "4-3"))
})
