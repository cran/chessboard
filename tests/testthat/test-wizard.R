...
test_that("wizard() - Tests for errors", {
  ...
  expect_error(wizard(nodes_qu),
               paste0("The wizard movement is not designed to work through ", 
                      "quadrats only. Please use pawn() instead."),
               fixed = TRUE)
  
  expect_error(wizard(nodes_tr),
               paste0("The wizard movement is not designed to work through ", 
...
test_that("wizard() - Tests for success (2D network)", {
  
  expect_silent({
    neighbors <- wizard(nodes,
                        focus    = "3-3",
                        degree   = 1,
                        directed = TRUE,
                        reverse  = FALSE, 
  ...
  expect_equal(class(neighbors), "data.frame")
  expect_equal(ncol(neighbors), 4L)
  expect_equal(nrow(neighbors), 14L)
  expect_equal(neighbors[ , "node"], c("1-3", "1-4", "1-5", "2-3", "2-4", "2-5",
                                       "3-4", "3-5", "4-3", "4-4", "4-5", "5-3",
                                       "5-4", "5-5"))
