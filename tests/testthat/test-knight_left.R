...
test_that("knight_left() - Tests for errors", {
  
  expect_error(knight_left(),
...
test_that("knight_left() - Tests for success (2D network)", {
  
  expect_silent({
    neighbors <- knight_left(nodes,
                             focus    = "3-3",
                             degree   = 1,
                             directed = TRUE,
                             reverse  = FALSE, 
  ...
  expect_equal(neighbors[ , "node"], c("1-4", "2-5"))
  ...
  expect_equal(ncol(neighbors), 4L)
  expect_equal(nrow(neighbors), 2L)
  expect_equal(neighbors[ , "node"], c("4-1", "5-2"))
  ...
  expect_equal(neighbors[ , "node"], c("1-4", "2-5", "4-1", "5-2"))
